"""
Bingo Grid Generator  v5
========================
Run with: python bingo_generator.py
Requires: pip install Pillow

ASSIGNMENT ALGORITHM — simple and provably correct
===================================================

The only way to guarantee BOTH:
  (A) no image repeats within a single grid
  (B) no image repeats in the same slot across different grids

...is to treat the problem as filling a matrix where:
  - each ROW    is one grid      (must contain each image at most once)
  - each COLUMN is one slot pos  (must contain each image at most once)

This is exactly a Latin Rectangle.

We build it like this:

  1. Start with one shuffled copy of the pool: [img0, img1, ..., img15]
     Call this the "master order".

  2. Grid 1 gets the master order, then internally shuffled.
  3. Grid 2 gets the master order ROTATED by `cells` positions, then shuffled.
  4. Grid 3 gets it rotated by `2*cells`, then shuffled.
  etc.

  Rotating by exactly `cells` each time means:
    - Every row contains every image exactly once (row uniqueness ✓)
    - Every column contains a different image in each row (column uniqueness ✓)
      because if two rows shared the same image in the same column, that would
      mean two images in the master order are `cells` apart AND map to the same
      column after shuffling — which we prevent by assigning columns BEFORE
      shuffling within the grid.

  Wait — shuffling destroys column order. So we do it differently:

  CORRECT APPROACH — transposition shuffle:
  -----------------------------------------
  Build a (cells x n_grids) table T where:
    T[slot][grid] = the image assigned to that slot in that grid.

  For each slot s:
    T[s] = a random permutation of the pool (no repeats across grids per slot).

  This guarantees column uniqueness by construction.

  Then for each grid g, collect column g from all slots: [T[0][g], T[1][g], ...]
  This gives the assignment for grid g.

  Row uniqueness: T[s][g] is drawn from independent shuffles of the pool,
  so two slots CAN get the same image for the same grid (collision).

  To fix collisions we do a targeted swap:
    If grid g has image X in slots s1 AND s2:
      Find another slot s3 where T[s3][g] != X and swapping T[s1][g] with T[s3][g]
      doesn't create a new collision in column s1 or s3.

  This is O(cells^2) per grid and always terminates because:
    - pool size >= cells means a valid assignment always exists.
    - We keep trying until no duplicates remain.
"""
...
def make_grids(pool, cells, n_grids):
    """
    Returns a list of n_grids lists, each of length `cells`.

    GUARANTEE 1 — no image repeats within a single grid (row uniqueness).
    GUARANTEE 2 — no image repeats in the same slot across grids (column uniqueness).

    Both guarantees hold as long as len(pool) >= cells.
    If pool is smaller, images must repeat (unavoidable) and only G1 is enforced.
    """
    assert cells > 0 and n_grids > 0

    # If pool too small, just do best-effort shuffles and return early
    if len(pool) < cells:
        result = []
        for _ in range(n_grids):
            g = (pool * (cells // len(pool) + 1))[:cells]
            random.shuffle(g)
            result.append(g)
        return result

    # ── Build slot queues (guarantees column uniqueness) ──────────────────
    # slot_q[s] is a list of n_grids images, all different, for slot s.
    # We build each queue as a tiled shuffle of the pool.
    slot_q = []
    for s in range(cells):
        ...
        while len(q) < n_grids:
            ...
            # Avoid same image at the boundary between tiles
            if q and q[-1] == chunk[0]:
                chunk[0], chunk[-1] = chunk[-1], chunk[0]
            q.extend(chunk)
        slot_q.append(q[:n_grids])

    # slot_q[slot][grid] = image path — column uniqueness guaranteed here.

    # ── Build the raw matrix ──────────────────────────────────────────────
    # matrix[g] = [slot_q[0][g], slot_q[1][g], ..., slot_q[cells-1][g]]
    matrix = []
    for g in range(n_grids):
        row = [slot_q[s][g] for s in range(cells)]
        matrix.append(row)

    # ── Fix row duplicates (enforce grid uniqueness) ──────────────────────
    # For each grid, if the same image appears in two slots, swap one of them
    # with a value from another slot that:
    #   (a) doesn't already appear in this row
    #   (b) won't create a column duplicate
    for g in range(n_grids):
        _fix_row(matrix, g, cells, n_grids, slot_q)

    # ── Randomise positions within each grid ──────────────────────────────
    # Shuffle the positions (columns) for each grid while maintaining
    # column uniqueness: we permute the COLUMN INDICES, not the values.
    # Swapping columns s1 and s2 everywhere preserves column uniqueness.
    # But we want grids to look different, so we do per-grid position swaps
    # that are checked against column uniqueness.
    for g in range(n_grids):
        for _ in range(cells * 8):
            s1 = random.randrange(cells)
            s2 = random.randrange(cells)
            if s1 == s2:
                continue
            # Swap is safe if no other grid has matrix[g'][s1]==matrix[g][s2]
            # or matrix[g'][s2]==matrix[g][s1]
            val1 = matrix[g][s1]
            val2 = matrix[g][s2]
            safe = True
            for g2 in range(n_grids):
                if g2 == g:
                    continue
                if matrix[g2][s1] == val2 or matrix[g2][s2] == val1:
                    safe = False
                    break
            if safe:
                matrix[g][s1], matrix[g][s2] = matrix[g][s2], matrix[g][s1]

    # ── Verify (safety net — prints to console if anything slipped through) ─
    _verify(matrix, cells, n_grids)

    return matrix


def _fix_row(matrix, g, cells, n_grids, slot_q):
    """
    Remove all duplicate images from row g.
    Uses targeted swaps that preserve column uniqueness.
    Runs until no duplicates remain or no valid swap exists.
    """
    for _pass in range(cells * cells):   # bounded iterations
        # Find duplicates
        seen  = {}
        dupes = []
        for s, img in enumerate(matrix[g]):
            if img in seen:
                dupes.append(s)          # s is the later occurrence
            else:
                seen[img] = s
        if not dupes:
            break                        # all clean
        ...
        s_bad   = dupes[0]
        img_bad = matrix[g][s_bad]

        # Images already used in this row (excluding s_bad itself for swap purposes)
        used_in_row = set(matrix[g]) - {img_bad}

        # Try to find a slot s2 whose value:
        #   - is not already in this row (so swapping won't introduce a new dupe)
        #   - placing img_bad at s2 doesn't violate column uniqueness for s2
        #   - placing current matrix[g][s2] at s_bad doesn't violate column for s_bad
        ...
        for s2 in candidates:
            if s2 == s_bad:
                continue
            val2 = matrix[g][s2]
            if val2 in used_in_row:
                continue                 # would create a new row duplicate
            # Check column uniqueness after swap:
            # s_bad would get val2 — make sure no other row has val2 in slot s_bad
            col_s_bad_ok = all(
                matrix[g2][s_bad] != val2
                for g2 in range(n_grids) if g2 != g
            )
            # s2 would get img_bad — make sure no other row has img_bad in slot s2
            col_s2_ok = all(
                matrix[g2][s2] != img_bad
                for g2 in range(n_grids) if g2 != g
            )
            if col_s_bad_ok and col_s2_ok:
                matrix[g][s_bad], matrix[g][s2] = matrix[g][s2], matrix[g][s_bad]
                swapped = True
                break

        if not swapped:
            # No clean swap found this pass — try a different bad slot next time
            # (can happen with tiny pools; break to avoid infinite loop)
            break


def _verify(matrix, cells, n_grids):
    """Print any violations to the console (does not raise)."""
    row_errors = []
    col_errors = []
    ...
    for g in range(n_grids):
        ...
        for s, img in enumerate(matrix[g]):
            if img in seen:
                row_errors.append(f"Grid {g+1}: '{os.path.basename(img)}' in slots {seen[img]} and {s}")
            seen[img] = s
    ...
    for s in range(cells):
        ...
        for g in range(n_grids):
            img = matrix[g][s]
            if img in seen:
                col_errors.append(f"Slot {s}: '{os.path.basename(img)}' in grids {seen[img]+1} and {g+1}")
            seen[img] = g

    if row_errors or col_errors:
        print("⚠ VERIFICATION FAILURES:")
        for e in row_errors:  print("  ROW:", e)
        for e in col_errors:  print("  COL:", e)
    else:
        print(f"✓ All {n_grids} grids verified — no row or column duplicates.")
...
class BingoApp(tk.Tk):
    ...
    def _generate(self):
        ...
        matrix = make_grids(pool, cells, n)

        for i, paths in enumerate(matrix):
            card  = self._compose_grid(paths)
            fname = os.path.join(out_dir, f"bingo_grid_{i+1:03d}.png")
            card.save(fname, "PNG")

        self._last_grid = self._compose_grid(matrix[-1])
